_DWORD *__thiscall sub_1022A6A0(_DWORD *this, char *Str, int a3)
{
  _DWORD *result; // eax
  char *v5; // eax
  char *v6; // esi
...
LABEL_10:
      if ( v15[7] )
        result = (_DWORD *)sub_1022A6A0(Str, 0);
      if ( !result )
      {
        if ( !(_BYTE)a3 )
          return 0;
        ...
        if ( v12 )
          result = sub_10229D20(v12, (int)v6);
        else
          result = 0;
        if ( v10 )
          v10[5] = result;
    ...
    if ( Stra )
      return (_DWORD *)sub_1022A6A0(Stra + 1, a3);
  }
  return result;
}
